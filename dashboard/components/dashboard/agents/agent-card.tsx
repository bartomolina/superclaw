      ...
      <div className="bg-white dark:bg-zinc-900/80 border border-zinc-200 dark:border-zinc-800/60 rounded-xl overflow-hidden shadow-sm dark:shadow-none">
        ...
        <div className="px-5 pb-4 space-y-2">
          ...
          <div className="rounded-lg border border-zinc-200 dark:border-zinc-800/60 bg-zinc-50/70 dark:bg-zinc-950/40 px-3 py-2.5 space-y-2">
            <div className="flex items-center gap-2 text-xs">
              <Shield size={11} className="text-zinc-400 shrink-0" />
              <span className="text-zinc-700 dark:text-zinc-300 font-medium">Kanban worker env</span>
              <span className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-zinc-200 text-zinc-500 dark:bg-zinc-800 dark:text-zinc-500">
                global
              </span>
            </div>
            <p className="text-[11px] leading-5 text-zinc-500 dark:text-zinc-400">
              Kanban workers now expect <span className="font-mono">KANBAN_BASE_URL</span> and{" "}
              <span className="font-mono">KANBAN_AGENT_TOKEN</span> from the global OpenClaw runtime. There are no per-agent Kanban overrides anymore.
            </p>
            {agent.sandboxed && (
              <p className="text-[11px] leading-5 text-zinc-400 dark:text-zinc-500">
                If this agent runs in a sandbox, mirror the same values into <span className="font-mono">agents.defaults.sandbox.docker.env</span> so the sandbox inherits them.
              </p>
            )}
          </div>
          <div className="flex items-center gap-2 text-xs">
